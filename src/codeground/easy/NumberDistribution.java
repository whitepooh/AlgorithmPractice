...
public class NumberDistribution {
	public static void main(String args[]) throws Exception {
		 ...
		 * 주석(//) 처리 하셔야 합니다.
		 */
		Scanner sc = new Scanner(new FileInputStream("NumberDistribution_input.txt"));
		...
		for (test_case = 1; test_case <= TC; test_case++) {
			...
			// 각 케이스 숫자의 개수
			int numOfEachCase = sc.nextInt();
			// 첫 숫자
			int first_num = sc.nextInt();
			// 결과값 저장할 변수
			int result = first_num ^ sc.nextInt();
			for (int caseOfNumber = 2; caseOfNumber < numOfEachCase; caseOfNumber++) {
